...
class Service extends CI_Controller {
	...
	function save_trans()
	{
		...
		log_message('info', 'Cabang insert result: ' . ($cabang_result ? 'success' : 'failed'));
	...
	public function ajax_cos_konf()
    {
        ...
        // Handle order - check if order array exists
        $order_column_index = 1; // Default to cos_kode
        $order_dir = 'asc';
        ...
        $columns = array(
            0 => 'costomer.id_costomer',
            1 => 'transaksi.cos_kode',
            2 => 'costomer.cos_nama',
            3 => 'costomer.cos_alamat',
            4 => 'costomer.cos_hp',
            5 => 'costomer.cos_tipe',
            6 => 'transaksi.follow_up_count',
            7 => 'transaksi.cos_kode' // For actions column
        );
    
        // Get total records count (without filtering)
        $total_records = $this->M_service->cos_konf_count_all();
    
        // Get filtered records count
        $total_filtered = $this->M_service->cos_konf_count_filtered($search);
        ...
        foreach ($data as $row) {
            ...
            $hp_wa = preg_replace('/\D/', '', $row->cos_hp);
            if (substr($hp_wa, 0, 1) == '0') {
                $hp_wa = '62' . substr($hp_wa, 1);
            }
    
            // WhatsApp button
            $wa_button = '<a href="https://wa.me/' . $hp_wa . '" target="_blank" class="flex items-center justify-center">
            ...
            // Follow Up button with count
            $follow_up_count = isset($row->follow_up_count) ? $row->follow_up_count : 0;
            $follow_up_button = '<div class="flex items-center justify-center">
                                    <button onclick="sendFollowUp(\'' . addslashes($row->cos_hp) . '\', \'' . addslashes($row->cos_nama) . '\', \'' . $row->trans_kode . '\')" class="w-8 h-8 flex items-center justify-center rounded-full bg-green-500 text-white hover:bg-green-600" title="Kirim Follow Up">
                                        <i data-feather="send" class="w-4 h-4"></i>
                                    </button>';
            
            if ($follow_up_count > 0) {
                $follow_up_button .= '<span class="ml-2 inline-flex items-center justify-center px-2 py-1 rounded-full text-xs font-medium bg-red-500 text-white">' . $follow_up_count . '</span>';
            }
            $follow_up_button .= '</div>';
    
            // Action dropdown
            $actions = '<div class="dropdown relative">
